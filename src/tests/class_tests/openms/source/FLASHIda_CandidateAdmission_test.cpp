...
// CA-B7: one anchor per species per survey. Non-strict inclusion runs the selection loop twice and
// phase 1 would otherwise come back for a second named charge in the same survey.
//
// This is the guard ADR-0036 narrows: today it refuses a second PeakGroup of the species
// unconditionally, and after the behaviour change it will refuse only one that adds nothing. The
// expectation here is TODAY's, deliberately -- it is what makes the extraction provable.
START_SECTION(a_species_takes_one_anchor_per_survey)
{
  PeakGroup pg = makeEnvelope(12351.39, {10, 13, 16}, 0.94, 10.0f);

  AnchorContext ctx = anchorCtx({10, 13, 16});
  ctx.already_this_survey = true;

  AnchorChoice a = pickAnchor(pg, ctx);
  TEST_EQUAL(a.resolved(), false)
  TEST_EQUAL(a.reason == AdmissionReason::AlreadyThisSurvey, true)
}
END_SECTION
