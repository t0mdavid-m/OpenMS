...
namespace OpenMS
{
  ...
  FLASHDeconvAlgorithm::FLASHDeconvAlgorithm(): DefaultParamHandler("FLASHDeconvAlgorithm"), ProgressLogger()
  {
    ...
    defaults_.setValue(
      "report_FDR", "false",
      "Report qvalues (roughly, point-wise FDR) for deconvolved masses. Decoy masses to calculate qvalues and FDR are also reported. Beta version.");
    ...
    mf_defaults.setValue("min_cos", -1.0,
                         "Cosine similarity threshold between avg. and observed isotope pattern.  When negative, MS1 cosine threshold for mass "
                         "deconvolution will be used ");
    mf_defaults.setValue(
      "mass_error_ppm", -1.0,
      "Feature tracing mass ppm tolerance. When negative, MS1 tolerance for mass deconvolution will be used (e.g., 16 ppm is used when -SD:tol 16).");
  ...
  int FLASHDeconvAlgorithm::getScanNumber(const MSExperiment& map, Size index)
  {
    ...
    if (map.getSourceFiles().empty()) { return (int)index + 1; }
