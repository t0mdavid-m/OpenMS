...
// --------------------------------------------------------------------------
#include <OpenMS/CHEMISTRY/FlashIdaProForma.h>
#include <sstream>
#include <stdexcept>
#include <iostream>
...
namespace OpenMS
{
    FlashIdaProForma::FlashIdaProForma(const AASequence& seq) : sequence_(seq) {}

    void FlashIdaProForma::validateCVModification(const std::string& modification)
    {
    ...
    void FlashIdaProForma::parseCVModificationNames(const std::string& modString, size_t& pos, size_t residue_pos)
    {
    ...
    void FlashIdaProForma::parseStandardModification(const std::string& modString, size_t& pos, size_t residue_pos)
    {
        size_t modStart = modString.find('[', pos);
        size_t modEnd = modString.find(']', modStart);
        if (modStart == std::string::npos || modEnd == std::string::npos)
    ...
    void FlashIdaProForma::parseDeltaMassNotation(const std::string& modString, size_t& pos, size_t residue_pos)
    {
      size_t modStart = modString.find('[', pos);
      size_t modEnd = modString.find(']', modStart);
      if (modStart == std::string::npos || modEnd == std::string::npos)
    ...
    void FlashIdaProForma::parseNTerminalModification(const std::string& modString, size_t& pos)
    {
    ...
    void FlashIdaProForma::parseCTerminalModification(const std::string& modString, size_t& pos)
    {
        size_t modStart = modString.find("-[", pos);
        size_t modEnd = modString.find(']', modStart);
        if (modStart == std::string::npos || modEnd == std::string::npos)
    ...
    void FlashIdaProForma::parseRangeModification(const std::string& modString, size_t& pos)
    {
    ...
    AASequence FlashIdaProForma::fromProFormaString(const std::string& proforma_str)
    {
      AASequence seq;
      size_t pos = 0;
    ...
    std::string FlashIdaProForma::toProFormaString() const
    {
      std::stringstream ss;
...
/////////////
    void FlashIdaProForma::throwParseError(const std::string& message) const
    {
      throw std::runtime_error("ProForma parsing error: " + message);
    }


    void FlashIdaProForma::removeModification(size_t position)
    {
    ...
    void FlashIdaProForma::addModification(size_t start_pos, size_t end_pos, const std::string& mod_id, double mass_shift)
    {
